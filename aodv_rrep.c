...
#define RREPENC_SIZE sizeof(RREPENC)
...
{
    RREP1 *rrp=NULL;
    rrp=malloc(sizeof(RREP));
    memcpy(rrp,rrep,sizeof(RREP));
	...
	if (neighbor && neighbor->state == VALID && !neighbor->ack_timer.used) {
     ...
     //log_pkt_fields((AODV_msg *)rre);
     //struct in_addr saddr,daddr;
     //daddr.s_addr=rre->dest_addr;
     //saddr.s_addr=rre->orig_addr;
     //printf("orgin add  %s\n",ip_to_str(daddr));
     //printf("Dest add  %s\n",ip_to_str(saddr));
     ...
     int keylength=128;
     RREPENC *encp;
     encp=malloc(sizeof(RREPENC));
     encp=(RREPENC *)aodv_socket_new_msg();
     encp->type = AODV_RREPENC;
     unsigned char aes_key[keylength/8];
     memset(aes_key, 0, keylength/8);
     size_t inputslength = sizeof(RREP);
    ...
    unsigned char encout[encslength];
    memset(encout, 0, sizeof(encout));
    unsigned char decout[inputslength];
    memset(decout, 0, sizeof(decout));
    AES_KEY enc_key,dec_key;
    AES_set_encrypt_key(aes_key, keylength, &enc_key);
    AES_cbc_encrypt((unsigned char *)rrp,encout, inputslength, &enc_key, iv_enc, AES_ENCRYPT);
    memcpy(encp->encout,encout,sizeof(encp->encout));
    AES_set_decrypt_key(aes_key, keylength, &dec_key);
    AES_cbc_encrypt(encout, decout, encslength, &dec_key, iv_dec, AES_DECRYPT);
    //RREP *dc= (RREP *)decout; 
   /* struct in_addr saddr1,daddr1;
    daddr1.s_addr=dc->dest_addr;
    saddr1.s_addr=dc->orig_addr;
    printf("Decrypted orgin add  %s\n",ip_to_str(saddr1));
    printf("Decrypted Dest add  %s\n",ip_to_str(daddr1));*/
    //printf("\n***********************************RREP ENCRYPTED******************************\n");
    aodv_socket_send((AODV_msg *)encp, rev_rt->next_hop,RREPENC_SIZE, MAXTTL,&DEV_IFINDEX(rev_rt->ifindex));
    //printf("\n*******************************************************************************\n");
			   ...
			   rt_table_t * fwd_rt, int ttl)
{
    ...
    int i=0;
    RREP *send=NULL;
    send=malloc(sizeof(RREP));
    RREPFWD *fwd;
    fwd=malloc(sizeof(RREPFWD));
    send=(RREP *) aodv_socket_queue_msg((AODV_msg *) rrep, size);
    send->hcnt = fwd_rt->hcnt;	
  /* Update the hopcount */
  /*--------------------------encryption-----------------------------------*/  
    int keylength=128;
    unsigned char aes_key[keylength/8];
    ...
    AES_KEY enc_key,dec_key;
    AES_set_encrypt_key(aes_key, keylength, &enc_key);
    AES_cbc_encrypt((unsigned char *)send,encout, encslength, &enc_key, iv_enc, AES_ENCRYPT);
    AES_set_decrypt_key(aes_key, keylength, &dec_key);
    AES_cbc_encrypt(encout, decout, encslength, &dec_key, iv_dec, AES_DECRYPT);
    struct in_addr saddr,daddr;
    RREP *dc= (RREP *)decout;
    daddr.s_addr=dc->dest_addr;
    saddr.s_addr=dc->orig_addr;
    printf("Decrypted orgin add  %s\n",ip_to_str(saddr));
    printf("Decrypted fwd dest add  %s\n",ip_to_str(daddr));
    log_pkt_fields((AODV_msg *)dc);
    fwd=(RREPFWD *)aodv_socket_new_msg();
    fwd->type=AODV_RREPFWD;
    memcpy(fwd->encout,encout,sizeof(fwd->encout));
    aodv_socket_send((AODV_msg *)fwd,rev_rt->next_hop,size,ttl,&DEV_IFINDEX(rev_rt->ifindex));
    precursor_add(fwd_rt, rev_rt->next_hop);
    precursor_add(rev_rt, fwd_rt->next_hop);
			   ...
			   unsigned int ifindex)
{
	...
	return;
    DEBUG(LOG_DEBUG, 0, "from %s about %s->%s",ip_to_str(ip_src), ip_to_str(rrep_orig), ip_to_str(rrep_dest));
#ifdef DEBUG_OUTPUT
          log_pkt_fields((AODV_msg *) rrep);
#endif
