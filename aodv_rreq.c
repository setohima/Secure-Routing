...
#define DEBUG_OUTPUT
#define RREQENC_SIZE sizeof(RREQENC)
#ifndef NS_PORT
...
RREQENC *NS_CLASS rreq_create(u_int8_t flags, struct in_addr dest_addr,
			   u_int32_t dest_seqno, struct in_addr orig_addr)
{
  ...
  DEBUG(LOG_DEBUG, 0, "Assembled RREQ %s", ip_to_str(dest_addr));
    ...
    log_pkt_fields((AODV_msg *) rreq);
     ...
     //Encryption phase
     RREQENC *enc;
     enc=malloc(sizeof(RREQENC));
     enc=(RREQENC *)aodv_socket_new_msg();
     enc->type = AODV_RREQENC;
    ...
    AES_cbc_encrypt((unsigned char *)rreq,encout, inputslength, &enc_key, iv_enc, AES_ENCRYPT);
    memcpy(enc->encout,encout,sizeof(enc->encout));
    return enc;    
}
			...
			int ttl, u_int8_t flags)
{
	...
	if (!DEV_NR(i).enabled)
	    continue;
	enc= rreq_create(flags, dest_addr, dest_seqno, DEV_NR(i).ipaddr);
	aodv_socket_send((AODV_msg *)enc, dest, RREQENC_SIZE, ttl, &DEV_NR(i));
    }
}
...
void NS_CLASS rreq_forward(RREQ * rreq, int size, int ttl)
{
     int i;
     RREQ *send=NULL;
     send=malloc(sizeof(RREQ));
     RREQFWD *fwd=NULL;
     fwd=malloc(sizeof(RREQFWD));
     struct in_addr dest, orig;
    dest.s_addr = AODV_BROADCAST;
    orig.s_addr = rreq->orig_addr;
    /* FORWARD the RREQ if the TTL allows it. */
    DEBUG(LOG_INFO, 0, "forwarding RREQ src=%s, rreq_id=%lu",ip_to_str(orig), ntohl(rreq->rreq_id));
    /* Queue the received message in the send buffer */
    send=(RREQ *)aodv_socket_queue_msg((AODV_msg *)rreq,size);
    send->hcnt++;  
    log_pkt_fields((AODV_msg *) send);
     int keylength=128;
     unsigned char aes_key[keylength/8];
     memset(aes_key, 0, keylength/8);
     int inputslength = 28;
    ...
    fwd=(RREQFWD*)aodv_socket_new_msg();
    fwd->type=AODV_RREQFWD;
    memcpy(fwd->encout,encout,sizeof(fwd->encout));
    //memcpy(enc->encout,encout,sizeof(enc->encout));
  
     /* Increase hopcount to account for  intermediate route */
    /* Send out on all interfaces */
      for (i = 0; i < MAX_NR_INTERFACES; i++)
      {
	if (!DEV_NR(i).enabled)
	    continue;
       aodv_socket_send((AODV_msg *)fwd,dest,RREQENC_SIZE,ttl,&DEV_NR(i));
    }
}
			   ...
			   unsigned int ifindex)
{
    ...
    while ((rreqlen - extlen) > RREQENC_SIZE) {
	switch (ext->type) {
	case RREQ_EXT:
