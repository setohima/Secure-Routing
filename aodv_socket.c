/*****************************************************************************
  *
 * Copyright (C) 2014 SRM University 
 *
 ...
 *
 * Authors: Sudharsan.D, <dcesudharsan@gmail.com>
 *          
...
#define SO_RECVBUF_SIZE 256*1024
        ...
        unsigned char encfwd[32];
        unsigned char decoutfwd[28];
    	unsigned char decout[28];
    	unsigned char ive[16];
    	unsigned char ivd[16];
        unsigned char raes[16];
        unsigned char renc[32];
    	unsigned char rdecout[20];
    	unsigned char rive[16];
    	unsigned char rivd[16];
        unsigned char rfwdaes[16];
        unsigned char rfwdenc[32];
    	unsigned char rfwddecout[20];
    	unsigned char rfwdive[16];
    	unsigned char rfwdivd[16];
					 ...
					 int ttl, unsigned int ifindex)
{
    ...
    /* Check what type of msg we received and call the corresponding
      ...
      neighbor_add(aodv_msg, src, ifindex);
        ...
        RREPENC *rrp=NULL;
        RREP *rp=NULL;
        rrp=(RREPENC *)aodv_msg;
        if(rrp->type==AODV_RREPENC)
       {
        int i=0;
        for(i=0;i<16;i++)
        {
        ...
        memcpy(renc,rrp->encout,sizeof(renc));
        AES_KEY dec_key;
        AES_set_decrypt_key(raes,128, &dec_key);
        AES_cbc_encrypt(renc,rdecout,32, &dec_key, rivd, AES_DECRYPT);
        rp=malloc(sizeof(RREP));
        rp=(RREP *)rdecout;
        rrep=(AODV_msg *)rp;
        }
        ...
        int rlen1=20;
        AODV_msg *rac=NULL;
        rac=malloc(sizeof(AODV_msg));
        RREPFWD *rfwd=NULL;
        RREP *ra=NULL;
        ra=malloc(sizeof(RREP));
        rfwd=(RREPFWD *)aodv_msg;
       if(rfwd->type==AODV_RREPFWD)
       {
        ...
        memcpy(rfwdenc,rfwd->encout,sizeof(rfwdenc));  
        AES_KEY dec_key;
        AES_set_decrypt_key(rfwdaes,128, &dec_key);
        AES_cbc_encrypt(rfwdenc,rfwddecout,32,&dec_key, rfwdivd, AES_DECRYPT);
        ra=(RREP *)rfwddecout;
        rac=(AODV_msg *)ra;
        struct in_addr saddr,daddr;
        daddr.s_addr=ra->dest_addr;
        saddr.s_addr=ra->orig_addr;
        printf("Decrypted packet rrep orgin add  %s\n",ip_to_str(saddr));
        printf("Decrypted fwd rrep dest add  %s\n",ip_to_str(daddr));
         }
 
    switch (aodv_msg->type) 
    {
    case AODV_RREQ:
         rreq_process((RREQ *)aodv_msg,len1, src, dst, ttl, ifindex);
        break;
    case AODV_RREQFWD:
        rreq_process((RREQ *)aodv_msg,len1, src, dst, ttl, ifindex);
        break;
    case AODV_RREPFWD:
	DEBUG(LOG_DEBUG, 0, "Received RREP FWD");
	rrep_process((RREP *)rac, rlen1, src, dst, ttl, ifindex);
	break;
    case AODV_RREPENC:
	DEBUG(LOG_DEBUG, 0, "Received RREP");
	rrep_process((RREP *)rrep, len2, src, dst, ttl, ifindex);
	break;    
    case AODV_RERR:
	...
	alog(LOG_WARNING, 0, __FUNCTION__,
	     ...
	     ip_to_str(src), ip_to_str(dst));
              log_pkt_fields((AODV_msg *)aodv_msg);
    }
}
...
void NS_CLASS recvAODVUUPacket(Packet * p)
{
    ...
    ttl = ih->ttl();
        ...
        memset(encfwd,'\0',sizeof(encfwd));
        memset(decoutfwd,'\0',sizeof(decoutfwd));
        memset(rfwdaes,'\0',sizeof(rfwdaes));
        memset(rfwdive,'\0',sizeof(rfwdive));
        memset(rfwddecout,'\0',sizeof(rfwddecout));
        memset(rfwdenc,'\0',sizeof(rfwdenc));
        memset(rfwdivd,'\0',sizeof(rfwdivd));   
       // memset(encfwd,'\0',sizeof(encfwd));
       // memset(decoutfwd,'\0',sizeof(decoutfwd));
...
static void aodv_socket_read(int fd)
{
	...
	return;
     ...
     if(aodv_msg->type==AODV_RREQENC)
{
        int len1=28;
        AODV_msg *ac=NULL;
        ac=malloc(sizeof(AODV_msg));
        RREQENC *r=NULL;
        RREQ *ra=NULL;
        ra=malloc(sizeof(RREQ));
        r=(RREQENC *)aodv_msg;
       if(r->type==AODV_RREQENC)
       {
        ...
        memcpy(enc,r->encout,sizeof(enc));             
        AES_KEY dec_key;
        AES_set_decrypt_key(aes,128, &dec_key);
        AES_cbc_encrypt(enc,decout,32,&dec_key, ivd, AES_DECRYPT);
        ra=(RREQ *)decout;
        ac=(AODV_msg *)ra;
        }
        aodv_socket_process_packet(ac,len1, src, dst, ttl, dev->ifindex);
        }
else if(aodv_msg->type==AODV_RREQFWD)
{
        int len1=28;
        AODV_msg *ac=NULL;
        ac=malloc(sizeof(AODV_msg));
        ...
        RREQ *ra=NULL;
        ra=malloc(sizeof(RREQ));
        fwd=(RREQFWD *)aodv_msg;
       if(fwd->type==AODV_RREQFWD)
       {
        ...
        memcpy(encfwd,fwd->encout,sizeof(encfwd));  
        AES_KEY dec_key;
        AES_set_decrypt_key(aes,128, &dec_key);
        AES_cbc_encrypt(encfwd,decoutfwd,32,&dec_key, ivd, AES_DECRYPT);
        ra=(RREQ *)decoutfwd;
        ac=(AODV_msg *)ra;
        }
 aodv_socket_process_packet(ac,len1, src, dst, ttl, dev->ifindex);
 			       ...
 			       int len, u_int8_t ttl, struct dev_info *dev)
{
	...
	sendPacket(p, dst, 0.0);
        ...
        if(send_buf[0]==1)
          {
           retval = sendto(dev->sock, send_buf, len, 0,(struct sockaddr *) &dst_addr, sizeof(dst_addr));
